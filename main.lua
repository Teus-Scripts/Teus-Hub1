...
local function getMeleeMastery(melee)
    local buoieltrul = game.Players.LocalPlayer.Backpack:FindFirstChild(melee) or game.Players.LocalPlayer.Character:FindFirstChild(melee)
    if buoieltrul and buoieltrul:FindFirstChild("Level") then
        return buoieltrul.Level.Value
    end
    return 0
end

local function checkMelee(melee)
...
local function buyMelee(melee)
    if table.find(meleetable, melee) then
        game.ReplicatedStorage.Remotes.CommF_:InvokeServer("Buy" .. melee)
    end
end
...
local function getMobLoadPos(a)
    for c, d in pairs(game:GetService("Workspace")["_WorldOrigin"].EnemySpawns:GetChildren()) do
        if d.Name == a then
            return d.CFrame
        end
...
local function CheckBoss(bg)
    if game.ReplicatedStorage:FindFirstChild(bg) then
        return game.ReplicatedStorage:FindFirstChild(bg)
    end
...
local function skidymf(a)
    if a and a.Parent and a:FindFirstChild("Human
