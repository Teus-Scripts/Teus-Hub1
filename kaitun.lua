...
-- Função para desativar a função de "shake" da câmera repetidamente
RunService.RenderStepped:Connect(function()
    local player = game.Players.LocalPlayer
    local camera = workspace.CurrentCamera

    if camera and camera.CameraType == Enum.CameraType.Scriptable then
        -- Definindo a CFrame da câmera para que fique fixo e não seja alterado por "shake"
...
-- Loop para enviar mensagem a cada 300 segundos
while true do
    wait(tempo)
    enviarMensagem()
end
...
-- Função que remove notificações de forma agressiva
local function removeNotifications()
    -- Verifica se o objeto de notificações existe e o remove
    local notifications = playerGui:FindFirstChild("Notifications")
    if notifications then notifications:Destroy() end

    -- Varre todos os descendentes e remove notificações ou textos específicos
    for _, obj in ipairs(playerGui:GetDescendants()) do
        if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("Frame") then
            if string.find((obj.Name or ""):lower(), "notification") or
               string.find((obj.Text or ""):lower(), "reward") or
               string.find((obj.Text or ""):lower(), "level up") then
                obj:Destroy() -- Destrói o objeto de interface encontrado
            end
        end
    end
end

-- Monitoramento contínuo para garantir a remoção
RunService.Heartbeat:Connect(function()
    pcall(removeNotifications)
end)

-- Monitora novos elementos adicionados e remove se forem notificações
